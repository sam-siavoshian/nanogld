...
def _derive_is_last_bar_of_day(bar_close_utc_ns: np.ndarray) -> np.ndarray:
    """Mark the final bar of each calendar day (UTC) as True.

    Used by the Gao-2014 baseline (hold position only on EOD per V1-SPEC
    §54). We bucket by integer UTC day; the last bar with each day key is
    flagged. Works for any RTH bar cadence in the unified dataset.

    Args:
        bar_close_utc_ns: (T,) int64 nanoseconds since UTC epoch.

    Returns:
        (T,) bool array, True only on the last bar of each UTC date.
    """
    bcn = np.asarray(bar_close_utc_ns, dtype=np.int64)
    # Calendar day key = floor-divide by ns-per-day (86_400_000_000_000).
    day_key = bcn // 86_400_000_000_000
    # Use end-of-array trick: a bar is the last of its day iff the next
    # bar has a different day key (or there is no next bar).
...
def run_model_on_fold(
    *,
    ...
    wf_cfg = cfg_yaml.get("walk_forward", {})
    n_folds = int(wf_cfg.get("n_folds", 4))
    dataset = NanoGLDDataset(
        ...
        fold_idx=fold_boundary.fold_idx,
        n_folds=n_folds,
    )
    if len(dataset) == 0:
        raise RuntimeError(
    ...
    # Pull real baseline inputs from unified + sidecar over the fold's
    # test slice. Prior to this fix, these arrays were all zeros which
    # caused gao_2014, xgboost, ma_cross, donchian, dlinear, etc to
    # return all-zero positions in the report.
    test_bar_idx = dataset._valid_indices  # absolute indices in unified
    unified_raw = torch.load(unified_path, map_location="cpu", weights_only=False)
    sidecar_raw = torch.load(sidecar_path, map_location="cpu", weights_only=False)
    features_full = unified_raw["features"]
    if isinstance(features_full, torch.Tensor):
        features_full = features_full.numpy()
    labels_full = unified_raw["labels"]
    if isinstance(labels_full, torch.Tensor):
        labels_full = labels_full.numpy()
    bcn_full = unified_raw["bar_close_utc_ns"]
    if isinstance(bcn_full, torch.Tensor):
        bcn_full = bcn_full.numpy()
    h5_full = sidecar_raw["gld_h5_log_return"]
    if isinstance(h5_full, torch.Tensor):
        h5_full = h5_full.numpy()
    h5x_full = sidecar_raw["gld_h5_x_vol_high"]
    if isinstance(h5x_full, torch.Tensor):
        h5x_full = h5x_full.numpy()
    GLD_CLOSE_FEATURE_IDX = 3  # confirmed via scripts/inspect_features.py
    close_test = features_full[test_bar_idx, GLD_CLOSE_FEATURE_IDX].astype(np.float64)
    last_bar_full = _derive_is_last_bar_of_day(bcn_full)
    is_last_bar_test = last_bar_full[test_bar_idx].astype(bool)
    # gld_h5_log_return is the 5th half-hour bar's log return per V1-SPEC §4
    # but only forward-filled to roughly the day boundary; the FINAL bar
    # of each session is often NaN. Gao-2014 trades on the EOD bar, so we
    # forward-fill h5 *within each UTC day* so the EOD bar carries the h5
    # value. Bars on a day with no h5 value at all remain NaN (and gao
    # will zero them as expected).
    day_key_full = bcn_full // 86_400_000_000_000
    h5_filled = h5_full.astype(np.float64).copy()
    last_day = None
    last_h5: float = float("nan")
    for i in range(len(h5_filled)):
        d = int(day_key_full[i])
        if d != last_day:
            last_h5 = float("nan")
            last_day = d
        v = h5_filled[i]
        if np.isfinite(v):
            ...
            h5_filled[i] = last_h5
    h5_test = h5_filled[test_bar_idx]
    # is_high_vol: gld_h5_x_vol_high is the interaction h5 × is_high_vol_day,
    # nonzero ONLY on the h5 bar (5th half-hour) of each high-vol day.
    # Gao-2014 trades on the EOD bar, so we must propagate the day's
    # high-vol indicator from the h5 bar across ALL bars in that day.
    day_key_full = bcn_full // 86_400_000_000_000
    h5x_nonzero = np.abs(h5x_full) > 1e-10
    hv_per_day: dict[int, bool] = {}
    for i in range(len(h5x_nonzero)):
        if h5x_nonzero[i]:
            hv_per_day[int(day_key_full[i])] = True
    is_high_vol_full = np.array(
        [hv_per_day.get(int(d), False) for d in day_key_full], dtype=bool
    )
    is_high_vol_test = is_high_vol_full[test_bar_idx].astype(bool)

    # XGBoost baseline needs a training slice for fitting. Use the fold's
    # train window from fold_boundary. labels in unified are int8 in
    # {0=DOWN, 1=FLAT, 2=UP}. We do NOT use the test labels — that would
    # leak. We do use the train slice features + labels directly.
    train_start, train_end = fold_boundary.train_start, fold_boundary.train_end
    train_features_arr = features_full[train_start:train_end].astype(np.float32)
    train_labels_arr = labels_full[train_start:train_end].astype(np.int64)
    test_features_arr = features_full[test_bar_idx].astype(np.float32)
    ...
    return {
        ...
        "close": close_test,
        "h5_log_return": h5_test,
        "is_last_bar_of_day": is_last_bar_test,
        "is_high_vol": is_high_vol_test,
        "train_features": train_features_arr,
        "train_labels": train_labels_arr,
        "test_features": test_features_arr,
    }
