...
class NanoGLDDataset(Dataset):
    ...
    def _compute_valid_indices(self, split: SPLIT_MODES) -> np.ndarray:
        ...
        When ``self.fold_idx is None``: use the static ``unified["splits"]``
        partition (V1 default; one train/val/test partition for the whole
        dataset; this is the legacy single-split behavior).

        When ``self.fold_idx`` is set: recompute splits in-memory using
        :func:`compute_fold_boundaries` over ``bar_close_utc_ns``. This
        gives proper 4-fold walk-forward CV with 1-week embargos between
        train/val/test windows (V1-SPEC §9.5).
        """
        n_bars = len(self._splits_arr)
        if split not in ("train", "val_a", "val_b", "val_c", "test"):
            raise ValueError(f"unknown split {split!r}")
        mode = split

        if self.fold_idx is None:
            # Legacy path: static partition from unified["splits"].
            if mode == "train":
            ...
            # Walk-forward path: recompute boundaries from timestamps.
            from nanogld.data.walk_forward_splits import (  # noqa: PLC0415
                compute_fold_boundaries,
            )

            ns = self._bar_close_utc_ns.cpu().numpy()
            folds = compute_fold_boundaries(ns, n_folds=self.n_folds)
            if not folds:
                raise RuntimeError(
                    f"compute_fold_boundaries returned 0 folds for {n_bars} bars"
                )
            if self.fold_idx < 0 or self.fold_idx >= len(folds):
                raise IndexError(
                    f"fold_idx={self.fold_idx} out of range; "
                    f"only {len(folds)} folds fit in this dataset"
                )
            fold = folds[self.fold_idx]
            mask = np.zeros(n_bars, dtype=bool)
            if mode == "train":
                mask[fold.train_start : fold.train_end] = True
            elif mode == "test":
                mask[fold.test_start : fold.test_end] = True
            else:
                val_len = fold.val_end - fold.val_start
                half = val_len // 2
                quarter = val_len // 4
                if mode == "val_a":
                    lo = fold.val_start
                    hi = fold.val_start + half
                elif mode == "val_b":
                    lo = fold.val_start + half
                    hi = fold.val_start + half + quarter
                else:
                    lo = fold.val_start + half + quarter
                    hi = fold.val_end
                mask[lo:hi] = True
            LOG.info(
                "WF fold %d mode=%s: train=[%d,%d) val=[%d,%d) test=[%d,%d) — picked %d bars",
                self.fold_idx,
                mode,
                fold.train_start,
                fold.train_end,
