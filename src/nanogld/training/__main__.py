...
def _build_dataloader(
    ...
    # Per-fold sidecar resolution. If `paths.sidecar_template` is set,
    # substitute `{fold}` to load the fold-specific sidecar (built by
    # `scripts/build_v1_sidecar.py --fold N`). Falls back to the static
    # `paths.sidecar` when no fold or no template is configured.
    sidecar_template = paths.get("sidecar_template")
    if sidecar_template and fold_idx is not None:
        sidecar_path = Path(str(sidecar_template).format(fold=fold_idx))
    else:
        sidecar_path = Path(paths["sidecar"])

    n_folds = int(cfg.get("walk_forward", {}).get("n_folds", 4))
    ablations = cfg.get("ablations", {}) or {}
    news_kill = bool(ablations.get("news_kill", False))
    ds = NanoGLDDataset(
...
def run(config_path: Path, fold: int, output_dir: Path, device: str = "cpu") -> int:
    ...
    llrd_cfg = LLRDConfig(
        ...
        sharpe_warmup_epochs=int(cfg["llrd"].get("sharpe_warmup_epochs", 0)),
        position_sat_check=bool(cfg["llrd"].get("position_sat_check", False)),
        position_sat_threshold=float(cfg["llrd"].get("position_sat_threshold", 0.95)),
