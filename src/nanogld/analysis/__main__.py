...
def _load_model(checkpoint_path: Path, device: str) -> nanoGLDV1:
    ...
    Auto-detect d_model + numeric_dim from the checkpoint so we can load
    Mac-mini reduced-scale checkpoints (d_model=192, t_bars=32) without
    ...
    # Detect dims from tensor shapes.
    kwargs: dict[str, int] = {}
    if "head.cls_head.weight" in sd:
        kwargs["d_model"] = int(sd["head.cls_head.weight"].shape[1])
    if "revin.affine_weight" in sd:
        kwargs["numeric_dim"] = int(sd["revin.affine_weight"].shape[0])
    if "patch_embed.pos_emb" in sd:
        num_patches = int(sd["patch_embed.pos_emb"].shape[1])
        if "patch_embed.proj.weight" in sd:
            patch_len = int(sd["patch_embed.proj.weight"].shape[1])
            t_bars = num_patches * patch_len
            kwargs["t_bars"] = t_bars
            kwargs["patch_len"] = patch_len
            kwargs["patch_stride"] = patch_len
    if "encoder.transformer_blocks.0.attn.head_gate" in sd:
        kwargs["num_heads"] = int(
            sd["encoder.transformer_blocks.0.attn.head_gate"].shape[0]
        )
    LOG.info("auto-detected ckpt dims: %s", kwargs)

    model = nanoGLDV1(**kwargs)
